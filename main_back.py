...
def main(save_image_mode, song_path):
    ...
    coord_queue = CoordQueue(tolerance=50)
    ...
    while True:
        ...
        if latest_frame is not None:
            ...
            # # ============= Normal Capture =============
            # else:
            #     cv2.imshow("Fake Osu", screenshot)q
            #     pass
        ...
        if current_action is None:
            ...
            if now_t >= (obj.time / 1000):
                # next_coord = coord_queue.pop()
                # print(next_coord)
                # click_x, click_y = next_coord

                if isinstance(obj, HitCircle):
                    click_x, click_y, q_class = coord_queue.pop("circle")
                    current_action = CircleAction(obj, x=click_x, y=click_y)
                    ...
                    current_action = SliderAction(obj, curr_uninherited_tp, sv, slider_multiplier, x=click_x, y=click_y)
...
if __name__ == "__main__":
    ...
    main(save_image_mode=False, song_path="./test_songs/cin_easy.osu")
